...
def extract_ids_and_classes_from_selectors(selectors):
    ...
    @return (ids, classes, origins) with:
        ...
        rest_selectors: list of selectors that don't contain ids or classes
    '''
    ids = set()
    classes = set()
    ...
    rest_selectors = set()
    for selector in selectors:
        extracted_ids = REGEX_ID.findall(selector)
        for id in extracted_ids:
            ids.add(id)
            origins['#' + id].append(selector)
        extracted_classes = REGEX_CLASS.findall(selector)
        for classs in extracted_classes:
            classes.add(classs)
            origins['.' + classs].append(selector)
        if len(extracted_ids) + len(extracted_classes) == 0:
            rest_selectors.add(selector)
    return (ids, classes, origins, rest_selectors)
    ...
    for css_file in css_files:
        ...
        if html_files:
            for html_file in html_files:
                ...
                results[css_file]['unused_selectors_after_dom_analysis'] = sorted(unused_selectors)


        # Extract ids and classes and scan other source files for these.
        if src_files:
            ids, classes, origins, rest_selectors = extract_ids_and_classes_from_selectors(unused_selectors)
            results[css_file]['ids'] = ids
            results[css_file]['classes'] = classes
            _log.info('Id/class extraction from %d CSS selectors for source code matching: extracted %d ids, %d classes.' % (len(unused_selectors), len(ids), len(classes)))
            ...
            original_total = len(unused_selectors)
            unused_selectors.difference_update(used_idclass_selectors)
            ...
            results[css_file]['unused_selectors_after_idclass_elimination'] = sorted(unused_selectors)
